...
export function ObraDetailPage({ obra }: ObraDetailPageProps) {
  ...
  // Refresh tasks silently on window focus.
  useEffect(() => {
    window.addEventListener("focus", refreshTasks);
    return () => window.removeEventListener("focus", refreshTasks);
  }, [refreshTasks]);

  // Refresh tasks silently on tab switch.
  useEffect(() => {
    refreshTasks();
  }, [activeTab, refreshTasks]);

  // WebSocket: patch the task in state the moment the chatbot updates it.
  useTaskSocket(
    obra.id,
    useCallback(
      (payload) => {
          ...
          prev.map((t) =>
            t.id === payload.taskId
              ? {
                  ...
                  estimated_progress: payload.estimatedProgress,
