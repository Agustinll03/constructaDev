...
async def create_obra(data: ObraCreate, db: DbSession, user_id: CurrentUserId):
    return await ObraService(db).create(data, user_id)


@router.get("", response_model=list[ObraSummary])
async def list_obras(db: DbSession, user_id: CurrentUserId):
    return await ObraService(db).list_mine(user_id)


@router.get("/{obra_id}", response_model=ObraRead)
async def get_obra(obra_id: int, db: DbSession, user_id: CurrentUserId):
    return await ObraService(db).get_for_manager(obra_id, user_id)


@router.patch("/{obra_id}", response_model=ObraRead)
async def update_obra(
    obra_id: int, data: ObraUpdate, db: DbSession, user_id: CurrentUserId
):
